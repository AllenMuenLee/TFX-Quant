...
## 2. 元大 API 前置作業（安裝本程式前完成）

元大提供兩套獨立的 API，**必須分別申請、分別安裝**。本安裝檔**不包含**這些元件
（券商元件無再散布權），只會偵測它們是否就緒。
...
2. 安裝程式會先檢查：Windows 版本、磁碟空間、VC++ x86 執行環境、以及元大交易／行情 API
   是否已安裝並註冊。**只有** Windows 版本與磁碟空間不足會中止安裝；其餘只會提示。
3. 預設安裝至使用者目錄（`%LOCALAPPDATA%\Programs\tfx-quant`），不需系統管理員權限。
4. 安裝程式會建立下列資料目錄（最小權限，僅目前使用者）：
